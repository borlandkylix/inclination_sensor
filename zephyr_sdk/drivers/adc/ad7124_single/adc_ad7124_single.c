...
#define DT_DRV_COMPAT adi_ad7124_single_adc
...
LOG_MODULE_REGISTER(adc_ad7124_single, CONFIG_ADC_LOG_LEVEL);
...
#define AD7124_STATUS_REG_CH_ACTIVE_MSK GENMASK(3, 0)
...
#define ADD7124_ADC_CTRL_CS_EN_MSK 		BIT(9)
#define ADD7124_ADC_CTRL_DATA_STATUS_MSK	BIT(10)
...
enum ad7124_filter_type {
	...
	AD7124_FILTER_SINC4_FAST_SETTLING = 4U,
	AD7124_FILTER_SINC3_FAST_SETTLING = 5U,
	AD7124_FILTER_POST_FILTER = 7U
};
...
struct adc_ad7124_config {
	...
	struct pinctrl_dev_config* spi_pin_cfg;
...
struct adc_ad7124_data {
	...
	K_KERNEL_STACK_MEMBER(stack, CONFIG_ADI_AD7124_SINGLE_ADC_ACQUISITION_THREAD_STACK_SIZE);
...
static int adc_ad7124_read_reg(const struct device *dev, uint32_t reg,
			       enum ad7124_register_lengths len, uint32_t *val)
{
	//TODO: need to use modifiable spi_dt_spec stored in data
	const struct adc_ad7124_config *config = dev->config;
	struct adc_ad7124_data *data = dev->data;
	//const struct spi_dt_spec *spec = &config->bus;
	const struct spi_dt_spec *spec = &data->bus;
	...
	case 2:
		*val = sys_get_be16(&buffer_rx[1]);
		break;
	case 3:
		if(add_status_length == 1) {
			*val = sys_get_be32(&buffer_rx[1]);
		}
		else{
...
static int adc_ad7124_filter_cfg(const struct device *dev, const struct ad7124_channel_config *cfg)
{
	...
	// filter_setup = FIELD_PREP(AD7124_FILTER_CONF_REG_FILTER_MSK, AD7124_FILTER_SINC4_FAST_SETTLING) |
	// 	       FIELD_PREP(AD7124_FILTER_FS_MSK, cfg->props.odr_sel_bits) |
	// 		   FIELD_PREP(GENMASK(16, 16), 1);
	// filter_mask = AD7124_FILTER_CONF_REG_FILTER_MSK | AD7124_FILTER_FS_MSK | GENMASK(16, 16);
...
static int adc_ad7124_set_cs_en(const struct device *dev, bool set_cs_en)
{
	int ret;

	ret = adc_ad7124_reg_write_msk(dev, AD7124_ADC_CONTROL, AD7124_ADC_CONTROL_REG_LEN,
				       FIELD_PREP(ADD7124_ADC_CTRL_CS_EN_MSK, set_cs_en?1:0),
				       ADD7124_ADC_CTRL_CS_EN_MSK);
	if (ret) {
		return ret;
	}
...
static int adc_ad7124_set_data_status(const struct device *dev, bool set_data_status)
{
	int ret;

	ret = adc_ad7124_reg_write_msk(dev, AD7124_ADC_CONTROL, AD7124_ADC_CONTROL_REG_LEN,
				       FIELD_PREP(ADD7124_ADC_CTRL_DATA_STATUS_MSK, set_data_status?1:0),
				       ADD7124_ADC_CTRL_DATA_STATUS_MSK);
	if (ret) {
		return ret;
	}
...
static int adc_ad7124_setup(const struct device *dev)
{
	...
	//TODO: force initial state to standby AD7124_STANDBY
	//ret = adc_ad7124_set_adc_mode(dev, config->adc_mode);
	ret = adc_ad7124_set_adc_mode(dev, AD7124_STANDBY);
	if (ret) {
		return ret;
	}
	...
	ret = adc_ad7124_set_data_status(dev, true);
	if (ret) {
		return ret;
	}
	//RDY signal only appears on the pin when CS is low,
	//so we have to keep CS low and cannot use CS_EN=1
	// ret = adc_ad7124_set_cs_en(dev, true);
	// if (ret) {
	// 	return ret;
	// }
	return 0;
...
static int adc_ad7124_perform_read_old(const struct device *dev)
{
	int ret = 0;
	struct adc_ad7124_data *data = dev->data;
	uint16_t ch_idx = -1;
...
void ad7124_data_ready(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	struct adc_ad7124_data *data= CONTAINER_OF(cb, struct adc_ad7124_data, gpio_drdy_cb);
	//LOG_DBG("data ready callback triggered, giving semaphore");
...
static int adc_ad7124_init_drdy_interrupt(const struct device *dev)
{
	int ret = 0;
	const struct adc_ad7124_config *config = dev->config;
	struct adc_ad7124_data *data = dev->data;
	...
	if(!gpio_is_ready_dt(&config->gpio_drdy)){
		LOG_ERR("nRDY gpio not ready");
		return -ENODEV;
	}

	ret = gpio_pin_configure_dt(&config->gpio_drdy, GPIO_INPUT | GPIO_ACTIVE_LOW);
	if (ret) {
		LOG_ERR("Failed to configure nRDY gpio");
		return ret;
	}

	gpio_init_callback(&data->gpio_drdy_cb, ad7124_data_ready, BIT(config->gpio_drdy.pin));
	ret = gpio_add_callback(config->gpio_drdy.port, &data->gpio_drdy_cb);
	if (ret) {
		LOG_ERR("Failed gpio_add_callback() %d", ret);
		return ret;
	}

	//need to restore default pinctrl otherwise drdy will stay as GPIO and SPI won't work.
	ret = pinctrl_apply_state(config->spi_pin_cfg, PINCTRL_STATE_DEFAULT);
	if (ret) {
...
static int adc_ad7124_wait_for_drdy(const struct device *dev)
{
	int ret = 0;
	const struct adc_ad7124_config *config = dev->config;
	struct adc_ad7124_data *data = dev->data;

	ret = pinctrl_apply_state(config->spi_pin_cfg, PINCTRL_STATE_PRIV_START);
	if (ret) {
	...
	// LOG_ERR("Time from single conversion start to wait for nRDY is %d us", k_cyc_to_us_near32(end_time - start_time));
	// int val = gpio_pin_get_raw(config->gpio_drdy.port, config->gpio_drdy.pin);
	// LOG_ERR("nRDY pin value 1st is %d", val);
	...
	// val = gpio_pin_get_raw(config->gpio_drdy.port, config->gpio_drdy.pin);
	// LOG_ERR("nRDY pin value 2nd is %d", val);
	...
	ret = gpio_pin_interrupt_configure_dt(&config->gpio_drdy, GPIO_INT_EDGE_TO_ACTIVE);
	if (ret) {
		LOG_ERR("Failed to configure nRDY gpio interrupt");
		return ret;
	}

	//wait for semaphore to be given in the callback when nRDY goes active
	//TODO: calculate the timeout properly based on ODR and enabled channels
	ret = k_sem_take(&data->data_ready, K_MSEC(10));
	if (ret) {
		LOG_ERR("k_sem_take() returns %d", ret);
		// do{	
		// 	val = gpio_pin_get_raw(config->gpio_drdy.port, config->gpio_drdy.pin);
		// 	LOG_ERR("nRDY pin value 3rd is %d", val);
		// 	k_msleep(1000);
		// }while(1);

	}
	ret = gpio_pin_interrupt_configure_dt(&config->gpio_drdy, GPIO_INT_DISABLE);
	if (ret) {
		LOG_ERR("Failed to configure nRDY gpio interrupt");
		return ret;
	}

	// ret = gpio_remove_callback(config->gpio_drdy.port, &data->gpio_drdy_cb);
	// if (ret) {
	// 	LOG_ERR("Failed to remove nRDY gpio interrupt");
	// 	return ret;
	// }
...
static uint16_t get_channel_ids(uint16_t ch_mask, uint16_t* buf, uint16_t buf_len)
{
	uint16_t ch_cnt = 0;
	for(uint16_t i=0; i<AD7124_MAX_CHANNELS && ch_cnt < buf_len; i++){
		if (ch_mask & BIT(i)){
			buf[ch_cnt++] = i;
		}
	}
...
static int adc_ad7124_perform_read(const struct device *dev)
{
	...
	//hold the enabled channel id and the num of it
	uint16_t channel_id[AD7124_MAX_CHANNELS]={0};
	uint16_t channel_cnt = 0;

	//get all enabled channel ids and store them in channel_id array
	channel_cnt = get_channel_ids(data->ctx.sequence.channels, channel_id, AD7124_MAX_CHANNELS);
	
	//use the idea from here:
	//https://github.com/zephyrproject-rtos/zephyr/blob/8a0b4d715ae5d8473da9e6fc98a49a041437fa46/drivers/adc/adc_max11102_17.c#L58
	//duplicate SPI configuration and set SPI_HOLD_ON_CS 
	// spi_operation_t op_buf = data->bus.config.operation;
	// data->bus.config.operation |= SPI_HOLD_ON_CS;

	k_sem_take(&data->acquire_signal, K_FOREVER);

	//single conversion kick off by writing to the ADC_CONTROL register
	// ret = adc_ad7124_set_adc_mode(dev, AD7124_SINGLE);
	// if (ret){
	// 	LOG_ERR("start single conversion failed");
	// 	data->bus.config.operation &= ~SPI_HOLD_ON_CS;
	// 	adc_context_complete(&data->ctx, ret);
	// 	return ret;
	// }
	//start_time = k_cycle_get_32();
	//loop to get data, using interrupt waiting for nRDY

	bool repeat_single_conv = false;
	...
	do{
		...
		ret = adc_ad7124_set_adc_mode(dev, AD7124_SINGLE);
		if (ret){
			LOG_ERR("adc_ad7124_set_adc_mode() returns %d", ret);
			error_occurred = true;
			break;
		}
		for(uint16_t i=0; i<channel_cnt; i++){
			//wait for each data ready signal
			ret = adc_ad7124_wait_for_drdy(dev);
			if (ret){
				LOG_ERR("adc_ad7124_wait_for_drdy() returns %d", ret);
				error_occurred = true;
				break;
			}
			if(i==channel_cnt-1){
				//last channel read, release CS after this read
				data->bus.config.operation &= ~SPI_HOLD_ON_CS;
			}
			ret = adc_ad7124_read_reg(dev, AD7124_DATA, AD7124_DATA_REG_LEN, &data_status);
			if (ret){
				LOG_ERR("adc_ad7124_read_reg() returns %d", ret);
				error_occurred = true;
				break;
			}

			adc_ch_id = data_status & AD7124_STATUS_REG_CH_ACTIVE_MSK;
			*data->buffer = data_status >> 8;

			if (channel_id[i] == adc_ch_id) {
				data->buffer++;
			} else {
				LOG_ERR("channel id mismatch, expected %d, actual %d", channel_id[i], adc_ch_id);
				if(adc_ch_id == channel_id[channel_cnt-1]){
					//LOG_DBG("restart ADC single conversion");
					//repeat_single_conv = true;
					error_occurred = true;
					ret = -EIO;
					break;
				}
	...
	}while(repeat_single_conv && !error_occurred);
	...
	// 	ret = adc_ad7124_wait_for_drdy(dev);
	// 	if (ret) {
	// 		LOG_ERR("waiting for data ready failed");
	// 		data->bus.config.operation &= ~SPI_HOLD_ON_CS;
	// 		adc_context_complete(&data->ctx, ret);
	// 		return ret;
	// 	}
...
static int adc_ad7124_init(const struct device *dev)
{
	...
	ret = adc_ad7124_init_drdy_interrupt(dev);
	if (ret) {
		LOG_ERR("adc_ad7124_init_drdy_interrupt() failed %d", ret);
		return ret;
	}
	...
	k_tid_t tid = k_thread_create(
		&data->thread, data->stack, CONFIG_ADI_AD7124_SINGLE_ADC_ACQUISITION_THREAD_STACK_SIZE,
		adc_ad7124_acquisition_thread, (void *)dev, NULL, NULL,
		CONFIG_ADI_AD7124_SINGLE_ADC_ACQUISITION_THREAD_INIT_PRIO, 0, K_NO_WAIT);
...
// see example from:
// https://github.com/zephyrproject-rtos/zephyr/blob/bb833c633138396fa239ed60ef5364694a7d09f9/drivers/sensor/broadcom/afbr_s50/afbr_s50.c#L414
// on how to refer to parent node's pinctrl configuration.
// Note that this also needs CONFIG_PINCTRL_DYNAMIC=y
#define ADC_AD7124_INST_DEFINE(inst)                                                       \
	...
	static const struct adc_ad7124_config adc_ad7124_config##inst = {                      \
		...
		.spi_pin_cfg = PINCTRL_DT_DEV_CONFIG_GET(DT_INST_PARENT(inst)),                    \
